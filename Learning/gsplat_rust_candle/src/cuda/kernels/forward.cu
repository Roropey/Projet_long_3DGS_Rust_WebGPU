...
extern "C" __global__ void nd_rasterize_forward(
    ...
    // paper)
    int idx;
    for (idx = ((int)range.x); idx < ((int)range.y); ++idx) {
...
extern "C" __global__ void rasterize_forward(
    ...
    // which gaussians to look through in this tile
    float2 range = tile_bins[tile_id];
    int num_batches = (((int) range.y) - ((int)range.x) + BLOCK_SIZE - 1) / BLOCK_SIZE;
    ...
    for (int b = 0; b < num_batches; ++b) {
        ...
        int batch_start =((int) range.x) + BLOCK_SIZE * b;
        ...
        int batch_size = min(BLOCK_SIZE,((int)range.y) - batch_start);
