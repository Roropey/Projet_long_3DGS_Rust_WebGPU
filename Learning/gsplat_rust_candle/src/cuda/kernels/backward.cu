...
extern "C" __global__ void nd_rasterize_backward_kernel(
    ...
    for (int idx = warp_bin_final - 1; idx >= (int) range.x; --idx) {
...
extern "C" __global__ void rasterize_backward_kernel(
    ...
    const int bin_final = inside? ((int) final_index[pix_id]) : 0;

    // have all threads in tile process the same gaussians in batches
    // first collect gaussians between range.x and range.y in batches
    // which gaussians to look through in this tile
    const float2 range = tile_bins[tile_id];
    const int num_batches = ((int) range.y - (int) range.x + BLOCK_SIZE - 1) / BLOCK_SIZE;
    ...
    for (int b = 0; b < num_batches; ++b) {
        ...
        const int batch_end = (int) range.y - 1 - BLOCK_SIZE * b;
        int batch_size = min(BLOCK_SIZE, batch_end + 1 - (int) range.x);
